document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchStartups() {
        ...
        try {
            ...
            console.log("Loaded Startups:", allStartupsData.length); // Debug Log
    ...
    function applyFilters() {
        ...
        if (selectedSDGs.length > 0) {
            filtered = filtered.filter(s => {
                // Handle different capitalization or field names
    ...
    function renderCards(startups) {
        ...
        startups.forEach(startup => {
            ...
            if (sSDGs && Array.isArray(sSDGs) && sSDGs.length > 0) {
                ...
                if (sSDGs.length > 1) tagsHTML += `<span class="tag small">+${sSDGs.length - 1}</span>`;
            }
